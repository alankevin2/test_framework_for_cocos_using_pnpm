...
function _encodePerson(message: Person, bb: ByteBuffer): void {
  ...
  let array$hobbies = message.hobbies;
  if (array$hobbies !== undefined) {
    for (let value of array$hobbies) {
      writeVarint32(bb, 26);
...
function _decodePerson(bb: ByteBuffer): Person {
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      // repeated string hobbies = 3;
      case 3: {
        let values = message.hobbies || (message.hobbies = []);
...
export function encodePersons(message: Persons): Uint8Array {
  let bb = popByteBuffer();
  _encodePersons(message, bb);
  return toUint8Array(bb);
}

function _encodePersons(message: Persons, bb: ByteBuffer): void {
  // repeated Person persons = 1;
  let array$persons = message.persons;
  if (array$persons !== undefined) {
    for (let value of array$persons) {
      writeVarint32(bb, 10);
      let nested = popByteBuffer();
...
export function decodePersons(binary: Uint8Array): Persons {
  return _decodePersons(wrapByteBuffer(binary));
}

function _decodePersons(bb: ByteBuffer): Persons {
  let message: Persons = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 1: {
        let limit = pushTemporaryLength(bb);
        let values = message.persons || (message.persons = []);
...
export function decodeMembership(binary: Uint8Array): Membership {
  return _decodeMembership(wrapByteBuffer(binary));
}

function _decodeMembership(bb: ByteBuffer): Membership {
  let message: Membership = {} as any;
